...
export class IPE {
	static async get(chat: Chat, prompt: string, agent) {
		const { metadata } = chat;
	...
	static async process(chat: Chat, message: string, answer: string, agent) {
		const response = await IPE.get(chat, message, agent);
