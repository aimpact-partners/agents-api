...
import { tickets } from '@aimpact/agents-api/data/model';
import { v4 as uuid } from 'uuid';

export /*bundle*/ class Tickets {
	static async create(params, user: User) {
		try {
			...
			const response = await tickets.set({ id: params.id, data });
	...
	static async get(id: string, user: User) {
		try {
			const response = await tickets.data({ id });
			if (response.error) return new BusinessResponse({ error: response.error });
	...
	static async byUser(user: User) {
		try {
			...
			const docs = await tickets.col().where('user.id', '==', user.id).get();
			const items = [];
	...
	static async all(user: User) {
		// TODO validate user's permissions
		try {
			const docs = await tickets.col().get();
			const items = [];
			docs.forEach(item => items.push(item.data()));
